#!/usr/bin/env python3
"""
Transform DT-Cargo tracks_with_energy.csv to REVOL-E-TION bev_log.csv for Depot Metzger.

Filters by freight_forwarder=1 (Metzger) and generates 15-min interval BEV usage log.
...
DEFAULT_START = "2021-09-10"  # First date in DT-Cargo data
DEFAULT_DAYS = 365
...
def sample_tracks_in_log(tracks: pd.DataFrame, log: pd.DataFrame, vehicle_ids: list, timestep: str) -> pd.DataFrame:
    ...
    # Set atac and atdc to False (no external/public charging - depot charging only)
    for i in range(len(vehicle_ids)):
        log.loc[:, (f"bev{i}", "atac")] = False
        log.loc[:, (f"bev{i}", "atdc")] = False
